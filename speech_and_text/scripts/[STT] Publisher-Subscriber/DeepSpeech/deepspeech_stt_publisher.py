...
def on_message(ws, message):
    print("Received message from deepspeech webserver:")
...
def on_open(ws):
    print("Connection to DeepSpeechSTT WebSocket succeded")
...
def deepspeech_stt_publisher():
    rospy.init_node('deepspeech_stt_publisher', anonymous=True)
    start_listening()

# Separated thread managing the WebSocket connected to the Docker deepspeech Server
def deepspeech_thread():
    global ws
    websocket.enableTrace(False)
    ws = websocket.WebSocketApp("ws://0.0.0.0:8767/",
...
if __name__ == '__main__':
    thread.start_new_thread(deepspeech_thread, ())
    deepspeech_stt_publisher()
